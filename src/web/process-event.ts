...
import { PluginsServer, Team } from 'types'
...
export class EventsProcessor {
    ...
    processEventFromKafka(message: Message, commit: () => void): void {
    ...
    processEventEE(
        ...
        if (data['event'] === '$snapshot') {
            ...
            */
            ...
            this._capture_ee(event_uuid, person_uuid, ip, site_url, team_id, data['event'], distinct_id, properties, ts)
        }
    }
    ...
    */
    ...
    _capture_ee(
        ...
        const { rows: [team] }: { rows: Team[] } = await this.pluginsServer.db.query(
            "SELECT slack_incoming_webhook, event_names, event_properties, event_names_with_usage, event_properties_with_usage, anonymize_ips FROM posthog_team WHERE id='t'"
        )
