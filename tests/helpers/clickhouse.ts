...
export async function resetTestDatabaseClickhouse(): Promise<void> {
    ...
    await clickhouse.query('TRUNCATE kafka_events').toPromise()
    await clickhouse.query('TRUNCATE kafka_person').toPromise()
    await clickhouse.query('TRUNCATE kafka_person_distinct_id').toPromise()
    await clickhouse.query('TRUNCATE kafka_session_recording_events').toPromise()
